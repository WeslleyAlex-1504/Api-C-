...
public class PagamentoModule : CarterModule
{
    ...
    public override void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/historico/aprovadas/{usuarioId}", async (int usuarioId, AppDbContext db) =>
        {
            var ordensAprovadas = await db.Pagamento
                .Where(p => p.Ordem.UsuarioId == usuarioId && p.Status == "approved")
                .Include(p => p.Ordem)
